...
class mBert_AEDA_Classifier():
    """Bert with regularisation in the form of AEDA. The goal is to compare results without AEDA"""
    ...
    def init_model(self):
        ...
        try:
            self.tokenizer = BertTokenizer.from_pretrained(model_name, cache_dir='/tmp/piedboef',
    ...
    def aeda_regularize(self, text_batch):
        rr=np.random.uniform(0, 1, size=len(text_batch))
        new_array=[]
        for ss, r in zip(text_batch, rr):
            if r>self.argdict['prob_reg']:
                ss=self.aeda(ss)
            new_array.append(ss)
        return new_array
    ...
    def run_epoch(self, train, dev, test, return_grad=False):
        ...
        self.model.train()
        bs=64 if not return_grad else 1
        ...
        for i, batch in enumerate(data_loader):
            ...
            text_batch=self.aeda_regularize(text_batch)

            encoding = self.tokenizer(text_batch, return_tensors='pt', padding=True, truncation=True, max_length=self.argdict['max_seq_length'])
