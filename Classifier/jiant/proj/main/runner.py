...
import Classifier.jiant.tasks.evaluate as evaluate
import Classifier.jiant.utils.torch_utils as torch_utils
from Classifier.jiant.proj.main.components.container_setup import JiantTaskContainer
from Classifier.jiant.proj.main.modeling.primary import JiantModel, wrap_jiant_forward
from Classifier.jiant.shared.constants import PHASE
from Classifier.jiant.shared.runner import (
    complex_backpropagate,
    get_train_dataloader_from_cache,
    get_eval_dataloader_from_cache,
)
from Classifier.jiant.utils.display import maybe_tqdm
from Classifier.jiant.utils.python.datastructures import InfiniteYield, ExtendedDataClassMixin
...
class JiantRunner:
    ...
    def evaluate_training_data(self, task_name_list, use_subset=None, return_preds=False, verbose=True):
        train_dict = {}
        train_dataloader_dict = self.get_train_data_loader_not_infinite()
        for task_name in task_name_list:
            task = self.jiant_task_container.task_dict[task_name]
            train_dict[task_name] = eval_train(
                train_dataloader=train_dataloader_dict[task_name],
    ...
    def get_train_data_loader_not_infinite(self):
        # Not currently supported distributed parallel
        train_dataloader_dict = {}
        for task_name in self.jiant_task_container.task_run_config.train_task_list:
            task = self.jiant_task_container.task_dict[task_name]
            ...
            train_dataloader_dict[task_name] = get_train_dataloader_from_cache(
                    train_cache=train_cache, task=task, train_batch_size=train_batch_size,
                )
...
def eval_train(
    ...
    for step, (batch, batch_metadata) in enumerate(
        maybe_tqdm(train_dataloader, desc=f"Eval ({task.name}, Train)", verbose=verbose)
    ):
        batch = batch.to(device)
        labels.extend(batch.label_id.tolist())
        with torch.no_grad():
            model_output = wrap_jiant_forward(
    ...
    output = {
        ...
        "metrics": evaluation_scheme.compute_metrics_from_accumulator(
            task=task, accumulator=eval_accumulator, tokenizer=tokenizer, labels=labels
        ),
    }
    if return_preds:
