...
class Bert_AutoSelect_Classifier():
	...
	def init_model(self):
		try:
			self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', cache_dir='/Tmp/piedboef', local_files_only=True)
		except:
		...
		self.gate_linear=torch.nn.Linear(64*768*2, 1).cuda()

	def gate(self, batch, training_set, split):
		text_batch = batch['sentence']
		labels=batch['label']
		encoding = self.tokenizer(text_batch, return_tensors='pt', padding='max_length', max_length=64, truncation=True)
		input_ids = encoding['input_ids'].cuda()
		input = F.one_hot(input_ids, num_classes=30522).float()
		embed = F.linear(input, self.model.bert.embeddings.word_embeddings.weight.t())
		# print(labels)
		if split=="train":
			text_aug=[]
			for lab in labels:
				random_ex=training_set.get_random_example_from_class(lab.item())
				text_aug.append(random_ex['sentence'])
			encoding_new=self.tokenizer(text_aug, return_tensors='pt', padding='max_length', max_length=64, truncation=True)
			input_ids_new = encoding_new['input_ids'].cuda()
			input_new = F.one_hot(input_ids_new, num_classes=30522).float()
			embed_new = F.linear(input_new, self.model.bert.embeddings.word_embeddings.weight.t())
			embed_gate=torch.cat([embed, embed_new], dim=-1)
			embed_gate=embed_gate.view(embed_gate.shape[0], -1)
			gated=self.gate_linear(embed_gate)
			gated=torch.sigmoid(gated)
			# print(gated.shape)
			# print(embed.shape)
			# print(gated.repeat(1, 3))
			embed_combined=embed*gated.unsqueeze(1).repeat(1, 64, 768)+embed_new*(1-gated.unsqueeze(1).repeat(1, 64, 768))
			return embed_combined
		else:
			return embed


	def run_epoch(self, train, dev, return_grad=False):
		"""Return grad returns the average grad for augmented and non augmented examples. """
		# train.return_pandas().to_csv("test.csv")
		self.model.train()
		bs=32 if not return_grad else 1
		...
		for i, batch in enumerate(data_loader):
			...
			input = F.one_hot(input_ids, num_classes=30522).float()
			embed = F.linear(input, self.model.bert.embeddings.word_embeddings.weight.t())
			embed=self.gate(batch, train, 'train')
			# fds
		...
		if return_grad:
			return accuracy_score(Y_train, pred_train), accuracy_score(Y_dev, pred_dev), sum(grad_og)/len(grad_og), sum(grad_aug)/len(grad_og), \
	...
	def train(self, train, dev, type='dataLoader', generator=None, return_grad=False):
		...
		for j in range(self.argdict['nb_epoch_classifier']):
			acc_train, acc_dev, grad_base, grad_aug, acc_base, acc_aug=self.run_epoch(train, dev, return_grad=return_grad)
			...
			accuracies.append((acc_train, acc_dev))
